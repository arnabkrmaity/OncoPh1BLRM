...
#' @title Combo2BLRMSim Function
#'
#' @description This function performs trial simulation with dual combination model
#'
#' @param Design   Dual combination design specification: contains required parametrs of \link[OncoPh1BLRM]{Combo2BLRM} (a list objest contains Agent1 (name of agent 1),
...
#' 
#' @seealso \code{\link{SingleBLRMSim}}, \code{\link{Combo2BLRM}}
#' 
#' @examples 
#' \dontrun{
#' # Design Parameters
...
Combo2BLRMSim <- function (Design =NULL,
                           nTrials = 1000,
                           TrueProbs= NULL,
                           StartDose = NULL,
                           ...
                           Decision= Combo2BLRMSimDecisionRule(MaxN = 60, mnMTD = 6, mnTOT = 15, mnTAR = 1, EWOC = 0.25,Escalation = "target", doseMaxEsc1 = 2, doseMaxEsc2 = 2),
...
#   Decision= OncoPh1BLRM:::Combo2BLRMSimDecisionRule(MaxN = Stop.rule$MaxN,
#                                       mnMTD = Stop.rule$mnMTD,
#                                       mnTOT = Stop.rule$mnTOT,
#                                       mnTAR = Stop.rule$mnTAR,
#                                       Escalation = Stop.rule$Escalation,
#                                       doseMaxEsc1 = Stop.rule$doseMaxEsc1,
#                                       doseMaxEsc2 = Stop.rule$doseMaxEsc2
#                                       )
...
Results <- do.call(foreach, forArgs) %dorng% {
    ...
    stop.trial <- FALSE
    too.toxic <- FALSE
    SimulationTrial <- matrix(nrow = 15, ncol = 23)
    colnames(SimulationTrial) = c("Trial", "Cohort", "Ntox",
                                  "Npat", "Weight", "DoseAdm1", "DoseAdm2", "under",
    ...
    pmix.eta= Design$pmix.eta
    pars=c("P12", "pCat","logAlphaBeta1", "logAlphaBeta2", "eta")
...
sink(paste(Outfile, "_log.txt", sep=""))
      ...
      while ((n.att <= 10) & !did.run) {
        ...
        currentRun <- try(Combo2BLRM(Data          = current.data,
                                     Agent1        = Agent1,
                                     Doses1        = Doses1,
                                     DoseRef1      = DoseRef1,
...
sink()
  ...
  if (NROW(simsummarys) == 0) {
    ...
    sim.sum.Vars <- names(formals(Combo2BLRMSimSummary))

    sim.sum <- do.call(Combo2BLRMSimSummary, mget(sim.sum.Vars))
