Combo2BLRM.Weight.WB <- function(){
  ...
  # Normal prior (or mixture normal prior) for interaction parameter (eta) 
  ...
  for (i in 1:Ncohorts){
    ...
    odds121[i] <-  odds121.0[i]*exp(eta*(DosesAdm1[i,1]/DoseRef1)*(DosesAdm2[i,1]/DoseRef2))
    P12.1[i] <- odds121[i]/(1 + odds121[i])
    
    #Using zero trick for the likelihood for any(Npat <= 1) || any(Ntox != round(Ntox)) || any(Npat != floor(Npat)
    
    #zero[i] <- 0
    minus.loglik[i] <- -Ntox[i,1]*log(P12.1[i]) - (Npat[i,1] - Ntox[i,1])*log(1 - P12.1[i])
